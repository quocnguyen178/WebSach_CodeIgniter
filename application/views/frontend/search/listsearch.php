<h1> <?php echo 'Kết quả tìm kiếm với từ khóa: '. $key; ?> <?php echo "sản phẩm: ".$total_rows; ?> </h1>
    </div>
    ...
    <?php if($key == '' || count($list) == 0){ echo '<p style=" font-size: 24px; color:red; margin-left:40px;"> Không tìm thấy kết quả </p>'; }else{ foreach($list as $arr){ ?>
