...
Class Danhmuc extends MY_Controller{
	...
	//hien thi san pham theo matheloai
	function listsanpham(){
		...
		$total_rows = count($this->sach_model->getsach_tl($id));
		$this-> data['total_rows'] = $total_rows;
		//load ra thu vien phan trang
		$this-> load-> library('pagination');
		$config = array();
		$config['total_rows'] = $total_rows;
		$config['base_url'] = base_url('frontend/danhmuc/listsanpham/'.$id);
		...
		$list = $this->db->where('matheloai',$id)->limit($config['per_page'],$segment)->get('sach')->result_array();

		$this-> data['list'] = $list;
		$this-> data['theloaiid'] = $this-> danhmuc_model->select_tlid($id);
		//hien thi ra view
		$this-> data['danhmuc'] = 'frontend/theloai/danhmuc';
		$this->load->view('frontend/layoutdanhmuc',$this-> data);
